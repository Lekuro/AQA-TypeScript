import { ApiPostsDto, ApiTodosDto, ApiUsersDto, ApiUserWithPostTodosDto } from './api-object.dto';

export class ApiUserWithPostAndTodos implements ApiUserWithPostTodosDto {
    ...
    public async getUser(): Promise<void> {
        try {
            const response = await fetch(ApiUserWithPostAndTodos.userUrl + this.id);
    ...
    public async getPosts(): Promise<void> {
        try {
            const response = await fetch(ApiUserWithPostAndTodos.postUrl + this.id);
            const jsonData = await response.json();
    ...
    public async getTodos(): Promise<void> {
        try {
            const response = await fetch(ApiUserWithPostAndTodos.todoUrl + this.id);
            const jsonData = await response.json();
